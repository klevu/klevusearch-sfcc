...
const Logger = LogUtils.getLogger('klevuSimpleProductsPriceDeltaExport');
...
var executePriceExportInFullExport = true;
/**
...
function process(record) {
    if (record && record.isVariant() !== true) {
        return productUtils.getMandatoryFields(record, isFullExport, executePriceExportInFullExport);
    }
}
...
function write(lines) {
    if (!empty(lines) && lines.length) {
        Logger.info('Found actual Simple Products Excluding Variation Product(s) that can be part of xml : ' + lines.length);
